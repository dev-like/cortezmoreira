        ...
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <h4 class="modal-title">Cadastrar Parceiro</h4>
                </div>
              {{ Form::open(['route' => 'parceiros.store', 'files' => true]) }}
                {{ csrf_field() }}
                    <div class="modal-body">
                      <div class="row">
                        <div class="form-group col-md-12">
                          {{ Form::label('imagem','Imagem') }}
                          <input type="file" name="imagem" id="profile-img" class="filestyle" data-buttonText="Carregar" data-placeholder="Cadastrar imagem do parceiro" data-btnClass="btn-light" required>
                        </div>
                      </div>
                      <div class="row">
                        <div class="form-group col-md-12">
                          {{ Form::label('texto', 'Texto') }}
                          {{ Form::textarea('texto', null, array('class' => 'form-control', 'autofocus','maxlength' => '250','required')) }}
                        </div>
                      </div>
                    </div>
    ...
    <table id="datatable" class="table table-bordered">
        ...
        <tbody>
          @foreach($parceiros as $parceiro)
            <tr>
              <td>{{ substr(strip_tags($parceiro->texto), 0, 150) }}{{ (strlen(strip_tags($parceiro->texto)) > 150 ? "..." : "") }} </td>
                <td width="15%">
                  <span class="hint--top" aria-label="Editar Unidade"><a href="{{ route('parceiros.edit', $parceiro->id) }}" style="border-radius: 50%" class="btn btn-warning waves-effect"> <i class="fa fa-pencil m-r-5"></i></a></span>
                  <span class="hint--top" aria-label="Deletar Unidade"><button type="button" onclick="goswet({{$parceiro->id}}, '{{$parceiro->rua}}')" style="border-radius: 50%" class="btn btn-danger waves-effect"> <i class="fa fa-trash m-r-5"></i></button></span>
                </td>
            </tr>
          @endforeach
        </tbody>
...
$(document).ready( function () {
  ...
  function goswet(id, nome){
    ...
    swal({
        title: "Deseja excluir o parceiro "+nome+"?",
        type: "warning",
